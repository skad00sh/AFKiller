...
- **Savings dashboard + event log.** Every cluster-stop is recorded in a persistent event log (`stats.toml`, capped at 500 entries) with timestamp, cluster id, DBUs credited, and auto/manual reason. Settings shows the latest 50 in a scrollable list plus this-week / this-month roll-ups alongside the existing lifetime total.
